...
## 🌐 在线同步版（GitHub Pages 网站）

除了本地桌面版，本仓库还发布了一个**在线网页版**，支持**多人在线实时同步**：

- 网站地址：**https://tangyuan9325.github.io/classroom-seat-arranger/**
- 老师在网页上排座/换座/轮换后点「保存并同步」，座位状态写入仓库；学生/家长访问同一网址即可**只读实时查看**，老师每次保存后 5 秒内自动刷新
...
- **贴合班主任要求的内置布局**：中间 4 列 × 6 行 + 旁边 4 列 × 5 行 = 44 座（第 6 排只坐中间），**两列一组**，女生占一个两列组（5+4，最后一名单独），其余 6 列男生。
- **固定座位（班主任硬性要求）**：沙宇桐 → 讲台旁单座；杨天雪 & 徐雨辰 → 固定同桌；张千慧 → 单人单座。可在软件中修改。
